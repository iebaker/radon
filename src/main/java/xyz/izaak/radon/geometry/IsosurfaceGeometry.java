...
public class IsosurfaceGeometry extends Geometry {
    ...
    public void build(Mesh mesh) {
        ...
        for (int i = 0; i < vertices.size(); i += 3) {
            ...
            mesh.next(VERTEX_POSITION, pointC);
            mesh.next(VERTEX_POSITION, pointB);
            mesh.next(VERTEX_POSITION, pointA);

            bMinusA.set(pointB).sub(pointA);
            cMinusA.set(pointC).sub(pointA);
            surfaceNormal.set(cMinusA).cross(bMinusA).normalize();

            mesh.next(VERTEX_NORMAL, surfaceNormal);
            mesh.next(VERTEX_NORMAL, surfaceNormal);
            mesh.next(VERTEX_NORMAL, surfaceNormal);
        }

        mesh.addInterval(GL_TRIANGLES, vertices.size());
    }
